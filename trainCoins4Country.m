...
%-------Run test-------------------------
    ...
    if plotTrainError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCoinPrediction(datasetDir, strcat(datasetDirRoot, imgDir), 'coin.tr.shuffled.restricted.csv', cnn, Theta3, Theta4, 100); %maxTestSamples = 100
    ...
    if plotValidationError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCoinPrediction(datasetDir, strcat(datasetDirRoot, imgDir), 'coin.cv.csv', cnn, Theta3, Theta4, maxTestSamples);
        fprintf('\n cv: Prediction error for coins : %2.3f%%\n', 100 - acc * 100);
        predError(trainingIter, 2) = 100 - acc * 100;
    end

    if plotTestError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCoinPrediction(datasetDir, strcat(datasetDirRoot, imgDir), 'coin.tst.csv', cnn, Theta3, Theta4, maxTestSamples);
