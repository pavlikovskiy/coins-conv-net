...
%datasetDir = 'C:/Develop/_chn-data/dataset-test-6-more/'; % dataset root dir
%addpath 'C:/Develop/_chn-data/dataset-test-6-more/';
datasetDir = 'C:/share/dataset-test-all2/'; % dataset root dir
trainSetCSVFile = 'country.tr.shuffled.restricted.csv'; % this file will be generated from 'coin.tr.csv'
...
if ~exist(strcat(datasetDir, 'country.tr.shuffled.csv'), 'file')
    fprintf('Generating shuffled training set country.tr.shuffled.csv from country.tr.csv \n');
    shuffleTrainingSet(datasetDir, 'country.tr.csv', 'country.tr.shuffled.csv');
end

if ~exist(strcat(datasetDir, 'country.tr.shuffled.restricted.csv'), 'file')
    fprintf('Generating shuffled restricted training set country.tr.shuffled.restricted.csv from country.tr.shuffled.csv \n');
 
    csvdata = csvread(strcat(datasetDir, 'country.tr.shuffled.csv'));  
    ...
    dlmwrite(strcat(datasetDir, 'country.tr.shuffled.restricted.csv'), shuffled_csvdata, 'precision',15);
    clear csvdata;
...
theta4File = strcat(datasetDir, tempDir, 'LFC2_THETA.mat');
if exist(theta4File, 'file')
    ...
    % File does not exist. random initialization
    fprintf('Cant load Thetta2 from %s  \n  Do random initialization for Theta2 \n', theta4File);
...
%}
    ...
    fprintf('\nIteration %4i done - Theta3, Theta4 saved. Average Cost is %4.4f \n', trainingIter, iterCost);
...
%-------Run test-------------------------
    ...
    if plotTrainError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCountryPrediction(datasetDir, imgDir, 'country.tr.shuffled.restricted.csv', cnn, Theta3, Theta4, 100); %maxTestSamples = 100
        fprintf('\n tr: Prediction error for countries (not for coins inside): %2.3f%%\n', 100 - acc * 100);
        predError(trainingIter, 1) = 100 - acc * 100;
    end

    if plotValidationError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCountryPrediction(datasetDir, imgDir, 'country.cv.csv', cnn, Theta3, Theta4, maxTestSamples);
        fprintf('\n cv: Prediction error for countries (not for coins inside): %2.3f%%\n', 100 - acc * 100);
        predError(trainingIter, 2) = 100 - acc * 100;
    end

    if plotTestError == 1 && trainingIter > startTestIteration
        [prediction, acc] = testCountryPrediction(datasetDir, imgDir, 'country.tst.csv', cnn, Theta3, Theta4, maxTestSamples);
        fprintf('\n tst: Prediction error for countries (not for coins inside): %2.3f%%\n', 100 - acc * 100);
